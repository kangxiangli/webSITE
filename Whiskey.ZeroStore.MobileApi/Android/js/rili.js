...
var CalendarHandler = {
    ...
    initialize: function() {
        ...
        $("#context").css("height",$(".week").width() + "px");
        ...
        $("#centerCalendarMain").css("height", $("#context").height() - 50 + "px").css("width", $("#context").width() + "px");
...
CalendarHandler.detailsshow();
    ...
    $('.item').bind('click', function () {
        ...
        if($(this).hasClass('lastItem')&&$(this).index()<=5){
            
            CalendarHandler.CalculateLastMonthDays();
         
          return;

        }else if($(this).hasClass('lastItem')&&$(this).index()>6){
           CalendarHandler.CalculateNextMonthDays();
         
          return;
        }
        else{
if ($('.dayItem').find('.addclose').length ==0) {
        //复制你的弹窗element
        var addCloseEle = $('.addclose').clone();
        select_index = $(this).index();
      }else{
             select_index = $(this).index();
            
        if(active==select_index){
      ...
      if (select_index >= 0 & select_index <= 6) {
        $(addCloseEle).css('margin-top',$(this).height());
        $(addCloseEle).insertAfter($(".item").eq(6)).slideDown();
      } else if (select_index >= 7 & select_index <= 13) {
      $(addCloseEle).css('margin-top',$(this).height()*2);
        $(addCloseEle).insertAfter($(".item").eq(13)).slideDown();
      } else if (select_index >= 14 & select_index <= 20) {
       $(addCloseEle).css('margin-top',$(this).height()*3);
        $(addCloseEle).insertAfter($(".item").eq(20)).slideDown();
      } else if (select_index >= 21 & select_index <= 27) {
       $(addCloseEle).css('margin-top',$(this).height()*4);
        $(addCloseEle).insertAfter($(".item").eq(27)).slideDown();
      } else if (select_index >= 28 & select_index <= 34) {
        $(addCloseEle).css('margin-top',$(this).height()*5);
        $(addCloseEle).insertAfter($(".item").eq(34)).slideDown();
      } else if (select_index >= 35 & select_index <= 41) {
     $(addCloseEle).css('margin-top',$(this).height()*6);
        $(addCloseEle).insertAfter($(".item").eq(41)).slideDown();
      }
