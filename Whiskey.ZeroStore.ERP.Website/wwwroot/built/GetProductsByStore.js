...
$(document).ready(function () {
    ...
    // 根据款号选中状态同步到数组中
    function addOrRemoveNum(bigProdNum, checked) {
        if (!bigProdNum || bigProdNum.length <= 0) {
            return;
        }
        var indexInArr = checkedNumbers.indexOf(bigProdNum);
        //移除或新增
    ...
    $("#nwe_table_1").off("click", "input[type=checkbox]")
        ...
        var bigprodnum = $(this).data("bigprodnumber");
        addOrRemoveNum(bigprodnum, checked);
    });
    $("#nwe_table_1").off("click", ".chk-all")
        .on("click", ".chk-all", function () {
        //选择后将bigprodnum存储在数组中
        if (_.every($("#nwe_table_1 tbody input[type=checkbox]"), function (chkbox) { return chkbox.checked; })) {
            // 全部取消勾选
            $("#nwe_table_1 tbody input[type=checkbox]").each(function (index, elem) {
                var bigprodnum = $(elem).data("bigprodnumber");
                var checked = false;
