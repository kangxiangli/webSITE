 ...
 * 获取商品款号信息
 * 
 * @interface GetProductsByNumsRes
 */
interface GetProductsByNumsRes {
    Id: number;
...
interface Window {
    initPaging(pageIndex: number): any;
    numCount: number;
    reloadTable(): any;
    plupload: any;

}
let checkedNumbers: Array<string> = [];
/**
 * uploader 配置
 */
$(function () {
    ...
    uploader.bind('FileUploaded', function (uploader, file, obj) {
        ...
        var res = JSON.parse($(obj.response).text()) as OperationResult<string[]>
...
$(function () {
    ...
    $(document).delegate(".clre", "click", function () {
        ...
        // 修改总数
        var totalCount = $("#tblCampCreate tbody tr").length as number;
        $("#productCount").text(totalCount);
        numCount--;
        if (0 == numCount % pageCount) {  //中间页然后如果删除的是这一页的最后一条 
            preLink();
            firstLink();
            nextText();
            lastText();
...
function initTable(bigProdNums) {
    ...
    if (bigProdNums != undefined && bigProdNums.length > 0) {
        ...
        if (bigProdNums.length == 0) return false;
        //totalCount += bigProdNums.length;

        // 请求新选择的款号信息,追加到table中
        $.post("/Properties/SalesCampaign/GetProductsByNums", { bigProdNums: bigProdNums }, function (da: OperationResult<GetProductsByNumsRes[]>) {
            var trs = "";
            if (da.ResultType == 3) {
