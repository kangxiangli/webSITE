...
(function () {
    ...
    $(function () {
        ...
        function select_check_Access() {
            ...
            var num = $("#hid-ordernum_dat").val();
            let tableData = window.gloablDataTableObj.data();
            var barcodes = [];
            ...
            $.whiskey.web.ajaxConfirm({
                ...
                success_event: function () {
                    ...
                    $checkboxes.each(function (index, elem) {
                        ...
                        // 获取选中的流水号
                        if ($(row).treegrid('isLeaf')) {
                            let barcode = $(row).treegrid("getNodeId");
        ...
        $("#save_pur").click(function () {
            //检测是否超时
            $.post("/Warehouses/Orderblank/IsTimeout", { orderblankNum: $("#hid-ordernum_dat").val(), action: OrderblankAction.Delivery })
            ...
            if (da.ResultType == 3) {
                ...
                $.whiskey.web.ajaxConfirm({
                    ...
                    success_event: function () {
                        ...
                        location.href = "/Warehouses/Orderblank/Index";
                    },
                });
    ...
    function removeBarcodes(barcodesFromUser: string[], orderblankNum: string, uuid: string) {
    ...
    function removeRow(trElement) {
        ...
        let orderblankNum = $("#hid-ordernum_dat").val();
        let barcodes = [];
        if ($(trElement).treegrid("isNode")) {
            if (!$(trElement).treegrid("isLeaf")) {
                ...
                //按流水号删除
                let barcode = $(trElement).find(":checkbox").val();
     ...
     */
    function updateValidCount(validCount: number) {
     ...
     */
    function updateInvalidCount(invalidCount: number) {
    ...
    //将数据压入队列
    function EnterQue(scanNumber:string) {
        ...
        if (checks.length !== 0) {
            ...
            for (var i = 0; i < checks.length; i++) {
                ...
                if ($(tr).treegrid("isLeaf")) {
                    let barcode = $(tr).treegrid("getNodeId");
