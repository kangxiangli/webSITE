    ...
    $(document).ready(function () {
        ...
        var dts = $("#nwe_table_1").dataTable({
            ...
            "fnServerParams": function (aoData) {
                //获取已经加到列表中的商品货号
                var productBarcodes = [];
                var selectedArr = $("#tableSelectionResult input[type=checkbox]:checked").each((index, elem) => {
                    let $tr = $(elem).parents("tr");
                    if ($tr.treegrid("isLeaf")) {
                        let barcode = $tr.find("span.pbarcode").text();
        ...
        $("#nwe_table_1").on('click', "input[type=checkbox]", function () {
            ...
            let $tr = $chkbox.parents("tr");
            if (!$tr.treegrid("isNode")) {
                return;
            }
            if (!$tr.treegrid("isLeaf")) {
                $tr.treegrid("getChildNodes").each((index, elem) => {
                ...
                if (!isChecked) {
                    ...
                    if (_.every($tr.treegrid("getParentNode").treegrid("getChildNodes").find("input[type=checkbox]"), elem => $(elem).prop("checked"))) {
