...

            {
                ...
                "mData": function (data) {
                    ...
                    if (data.OrderBlankNumber && data.OrderBlankNumber.length > 0) {
                        var stat = data.Status as OrderblankStatus;
                        switch (stat) {
    ...
    $('.toggle-cancel').switcher({
        ...
        $(".table-orderblank")
            .delegate(".order-view,.barcodeCount", "click", function () {
                let orderblankId = $(this).parents("tr").find(":checkbox").val();
            ...
            .delegate(".btnRemove", "click", function () {
                ...
                $.post("/Warehouses/Orderblank/IsTimeout", { orderblankNum: ordernum, action: OrderblankAction.Delete })
                    .done(function (data: OperationResult<string>) {
            ...
            .delegate(".reject_ord_but", "click", function () {
                ...
                //拒绝配货
                var ordernum = $(this).parents("tr").children("td").eq(2).text();
                $.post("/Warehouses/Orderblank/IsTimeout", { orderblankNum: ordernum, action: OrderblankAction.Reject })
                    .done(function (data: OperationResult<string>) {
                        if (data.ResultType != 3) {
                            $.whiskey.web.alert({
                                type: "danger",
            ...
            .delegate(".checkedok_but", "click", function () {
                ...
                $.post("/Warehouses/Orderblank/IsTimeout", { orderblankNum: num, action: OrderblankAction.Accept }).done(function (data: OperationResult<string>) {
                    if (data.ResultType != 3) {
                        $.whiskey.web.alert({
                            type: "danger",
                            par: { keyboard: false, backdrop: "static" },
         ...
         */
        function confirmAccept(orderbalnkNum: string) {
...
//												}    
     ...
     * @returns {string}
     */
    function generateBtn(data: any): string {
        var btn = "<button title='详细信息' class='btn btn-xs order-view btn-padding-right'><i class='fa fa-eye'></i></button>";
        let stat = data.Status as OrderblankStatus;
        let isSenderClosed = data.isSenderClosed;
        let isReceiverStoreClosed = data.isReceiverStoreClosed;
        switch (stat) {
            case OrderblankStatus.配货中: {
