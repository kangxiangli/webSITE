/**
 * Created by xiuxia on 2016/5/18.
 */
var macAdd = "";
//初始化页面的高度
...
$("#content_system").height(cont2 - $(".systemBox").height() - 2);
$(".content_list").outerHeight(cont2 - inp_hg);
$("#wrapperUl").outerHeight(wh - $(".staff_totle").height() - $(".change_date").height() - $(".stat_til").height());
$("#send,#history").height($(window).height() - $(".person_infi").height() - $(".send_history").height() - $(".foot_apply").height() - 2);
$("#info_msg,#historyNews").height($(window).height() - $(".person_infi").height());
$("#smallBox").height($("#linkMan").height() - $(".sSp").height() - $(".headOffice").height() - 5);
...
wol();
//初始化消息(通知)
//setInterval("getInfos()", 5000); //定时刷新消息数据
getInfos();

//显示消息
function tilShow(txt) {
    $(".btn_qd .tilBtn").html(txt).show();
    var tilBtn = $(".btn_qd .tilBtn").width();
    $(".btn_qd .tilBtn").css("margin-left", -(tilBtn / 2));
...
function magCen() {
    var btnTil = $(".tilBtn").width();
    $(".tilBtn").css("margin-left", -(btnTil / 2));
}
$(".dk_time .da_btn").click(function () {
    var valu = $(".xx_ft span").attr("data-init");
    if (valu == 1) {
        showMsg("请先阅读未读消息,再打卡");
    } else {
    ...
    $.xiaodie.ajax({
        ...
        success: function (data) {
            ...
            if (data.ResultType == 3) {
                ...
                $(".til_dk .kq_tx").attr("src", $.xiaodie.host+userPhoto);
                $(".dk_time .dp").html(depHtml + depName);
                $(".dk_time .header_img").attr("src", $.xiaodie.host+userPhoto);
        ...
        success: function (data) {
            if (data.ResultType == "3") {
                ...
                for (var i = 0; i < contacts.length; i++) {
                    var con = contacts[i];
                    var img = '<img class="tx_hd" src="' +$.xiaodie.host+con.UserPhoto + '" alt="头像" />';
        ...
        success: function (data) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                for (var j = 0; j < admins.length; j++) {
                    var admin = admins[j];
                    var img = '<img class="tx_hd" src="' + ($.xiaodie.host + admin.UserPhoto) + '" alt="头像" />';
...
$(function () {
    ...
    $("#right_box_shade,.shouYe").click(function () {
        animatend();
    });
    function animatend() {
    ...
    $(".da_footer>div").click(function () {
        var n = $(this).index();
        $(this).css({ "opacity": "1", 'color': 'rgba(255,255,255,1)' });
        $(this).siblings().css({ "opacity": "0.55", 'color': 'rgba(255,255,255,0.55)' });
        //当n=2时，表示点击了通讯按钮
        if (n == 1) {
            //获取当前时期并加载考勤数据
            initAttenByDate();
        ...
        if (n == 2) {
            ...
            if (EnablePhone == "true") {
                ...
                showMsg("添加手机号才能访问");
                $(".prompt").click(function () {
                    $(".xx_ft").css("opacity", "1").css("color", "#fff");
                    $("#content_3").css("display", "block");
                    $("#content_3").siblings().css("display", "none");
                });
                $(this).css("opacity", "0.55").css("color", "rgba(255, 255, 255, 0.5)");
                return false;
            }
        }
        if (n == 3) {
        ...
        success: function (data) {
            ...
            for (var i = 0; i < dataNum.length; i++) {
                if (dataNum[i].Status == 0) {
                    var optionLi = '<li onclick="getInform(this)" data-init=' + dataNum[i].Id + '><p><span>' + dataNum[i].Title + '</span><b></b></p><i>' + tim(dataNum[i].SendTime) + '</i><h6>' + dataNum[i].Content + '</h6><input type="hidden" value=' + dataNum[i].RealName + '></li>'
                    numInfo++;
                } else {
                    var optionLi = '<li onclick="getInform(this)"  data-init=' + dataNum[i].Id + '><p><span>' + dataNum[i].Title + '</span></p><i>' + tim(dataNum[i].SendTime) + '</i><h6>' + dataNum[i].Content + '</h6><input type="hidden" value=' + dataNum[i].RealName + '></li>'
                }
                $("#message ul").append(optionLi);
            }
            $(".system .num_email").html(numInfo);  //未读通知个数
        }
    });
    //获取未读消息
    $.xiaodie.ajax({
        ...
        success: function (data) {
            ...
            if (numNews != 0 || numInfo != 0) {
                $(".xx_ft").append('<span class="newEml" data-init="1"></span>');
            } else {
                $(".xx_ft span").remove();
            }
        }
        ...
        success: function (data) {
            ...
            for (var i = 0; i < infoNum.length; i++) {
                ...
                if (infoNum[i].Status != 0) {
                   ...
                   liContent = '<li class="content_info">' + infoNum[i].Content + '</li>',
                   liTime = ' <li class="tx3_dj">' + tim(infoNum[i].SendTime) + '</li>';
                    $(".newsBox").append(liHead + imgSend + ulHead + liTille + liSender + liContent + liTime + ulEnd + liEnd);
                }
            }
        }
    });
    $("#newsInfo .back_ig").click(function () {
        $("#newsInfo").animate({ "left": "150%" });
    });
});


//发送邮件
$(".officeLi h4 .launch").click(function () {
    var hl = $(this).html();
    if (hl == '+') {
        $(this).html("-");
    } else {
    ...
    $(this).parents().siblings(".peoBox").toggle();
})
//这里是获取焦点时的事件
    ...
    $.xiaodie.ajax({
        ...
        success: function (data) {
            if (data.ResultType == 3) {
                ...
                $(".send_history div").eq(0).removeClass("bgWh");
                $(".send_history div").eq(1).addClass("bgWh");
                $("#send").hide();
                $("#history").show();
                hisInfo();
                getInfos()
...
function getInform(op) {
    ...
    var til = $c.children("p span").html(),
        ...
        valPeo = $t.children("input").val();
    getAjax(adminId, $d, 1);
    console.log(valPeo);
    $("#article_inf b").html(til);
    $("#article_inf .mes_cont").html(cot);
    $("#article_inf .sedPeo").html(valPeo);
