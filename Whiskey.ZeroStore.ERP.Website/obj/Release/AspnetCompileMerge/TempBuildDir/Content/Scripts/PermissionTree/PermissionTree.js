(function ($) {
    ...
    function initialTree() {
        ...
        $(':checkbox[data-gtype="' + group.chakan + '"],:checkbox[data-gtype="' + group.shanchu + '"],:checkbox[data-gtype="' + group.jinyong + '"]').parent().parent().hide();
    ...
    function initialCheckBoxCheckedStatus() {
        ...
        $(".sanji_border").each(function (i, item) {
            var sanjiChecked = $(item).find("li.sanji> span >:checkbox:checked").length;
            var sanjiCheckedLength = $(item).find("li.sanji> span >:checkbox").length;
            if (sanjiCheckedLength == sanjiChecked) {
                $(this).siblings("span").attr("style", "color:red !important");
            }
        })
        $(".yiji>ul").each(function (i, item) {
            var erjiLength = $(item).find(".erji").length;
            var num = 0;
            $(item).find(".erji").each(function (j, items) {
                if ($(items).find("span").css("color") == "rgb(255, 0, 0)") {
                    num++;
                    if (num == erjiLength) {
                        $(item).siblings("span").attr("style", "color:red !important");
                    }
                } else {
                    return;
                }
        ...
        $(".tree li.parent_li>span>:checkbox").bind("click", function () {
            ...
            if (this.checked) {
                $(this).parent().attr("style", "color:red !important");
                if ($(this).has(".erji") || $(this).has(".sanji")) {
                    $(this).parents(".parent_li:eq(0)").find("li.erji>span").attr("style", "color:red !important");
                }
            } else {
                $(this).parent().attr("style", "color:none");
                $(this).parents(".parent_li:eq(0)").find("li.erji>span").attr("style", "color:none");
            }
            changecheckedFunc(this);
        });
        ...
        function changecheckedFunc(obj) {
            ...
            var childrenCount = $(obj).parents("li:eq(0)").find(".parent_li>span :checked");
            if (parentCb.length) {
                ...
                if (CbCheckedCount == CbAll.length && CbCheckedCount > 0) {
                    parentCb.find(">span").attr("style", "color:red !important");
                } else {
                    parentCb.find(">span").attr("style", "color:none");
                }
