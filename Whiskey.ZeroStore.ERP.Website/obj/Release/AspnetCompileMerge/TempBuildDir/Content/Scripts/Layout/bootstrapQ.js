...
$.fn.bstree = function (options) {
    ...
    bootstrapQ.tree.init(opt);
    $(this).find(":checkbox:checked").each(function () {
        var val = $(this).val();
        var reg = /^c.*/gi;
        if (!reg.test(val))
            $(this).parents("a:first").find("span").addClass('glyphicon-plus').removeClass('glyphicon-minus');
    });
    return true;
};
...
bootstrapQ.tree.sub = function (tree, opt) {
    ...
    if (tree) {
        ...
        if (opt.checkbox) {
            if (tree.msg != undefined && tree.msg != null && tree.msg != "" && tree.msg != "null") {
                res += '<label title=' + tree.msg + '>';
            }
            else {
                res += '<label >';
            }
            res += '<input type="checkbox" value=' + tree.id + ' class="treecheckbox" ';
            if (tree._checked) {
                res += 'checked';
            }
            var reg = /^c.*/gi;
...
bootstrapQ.tree.addchild = function (par, opt) {
    var url = opt.url.substr(0, opt.url.lastIndexOf('/'));
    var parid = $(par).parents('a').qdata().id;
    var reg = /^\d+$/;
    if (!reg.test(parid)) return;
    var json = qiao.ajax({ url: url + '/GetChilById?_pid=' + parid }); //yxk
    if (json && json.obj && json.obj.length > 0) {

        var tree = json.obj;
        var res = "";
        for (var i = 0; i < tree.length; i++) {
            res += "<label title=" + tree[i].msg + "><input type='checkbox' name='right' class='cl' ";
            if (tree[i]._checked) {
                res += "checked='checked' "
            }
            res += "value='" + tree[i].id + "'>" + tree[i].text + "</label>"
        }
        //var res = '<ul style="padding-left:20px;" id="treeid_' + parid + '" class="nav collapse ';
        //    if (opt.open)
        //        res += 'in';
        //    res += '">';
        //    for (var i = 0; i < tree.length; i++) {
        //        res += bootstrapQ.tree.sub(tree[i], opt);
        //    }
        //    res += '</ul>';
        res = "<ul id=treeid_" + parid + " class='pri-ul-li' style='list-style-type:none'><li>" + res + "</li></ul>"
        $(par).parents('li:first').append(res);
        bootstrapQ.tree.init(opt);
        $(par).click();
    }
    else {
        $(par).removeClass('glyphicon-minus').removeClass('glyphicon-plus');
    }
}
bootstrapQ.tree.init = function (opt) {
    qiao.on('#treeul .glyphicon-minus', 'click', function () {
        if ($(this).parent().next().length > 0) {
            $('#treeid_' + $(this).parents('a').qdata().id).hide(400);//collapse('hide');
            $(this).removeClass('glyphicon-minus').addClass('glyphicon-plus');
        }
    });
    qiao.on('#treeul .glyphicon-plus', 'click', function () {
        if ($(this).parent().next().length > 0) {
            $('#treeid_' + $(this).parents('a').qdata().id).collapse('show').show(600);
            $(this).removeClass('glyphicon-plus').addClass('glyphicon-minus');
        }
        else {
            bootstrapQ.tree.addchild(this, opt);
        }
    });
    //yxk 2015-11-24
    qiao.on('input.treecheckbox', 'change', function () {
        ...
        //$(this).parents("ul:first").parents("li:first").find('input.treecheckbox').prop('checked', subFlag);
        var conti = true;
        var send = this;
        if (subFlag) {
            var tit = $(send).parent('label').attr("title");
            //if (tit == undefined || tit == null) {
            //    $(send).parents("a:first").find("span").addClass('glyphicon-plus').removeClass('glyphicon-minus');
            //}
            $(send).parents("a:first").find("span").addClass('glyphicon-plus').removeClass('glyphicon-minus');
            for (var i = 0; conti; i++) {
                send = $(send).parents("ul:first").parents("li:first").children('a').find('input.treecheckbox');
                if (send.length == 0)
                    conti = false;
            ...
            $(send).parents("a:first").find("span").removeClass('glyphicon-plus').removeClass('glyphicon-minus');


            $('#treeid_' + $(this).parents('a').qdata().id).hide(400);//collapse('hide');
            $(send).parents("li").first().children("ul").find(":checkbox:checked").prop('checked', false);




            //$(send).parents("a:first").parent("li").find("ul").removeClass("in");
            //for (var i = 0; conti; i++) {
            //    var cur = $(cursend).parents("ul:first").find('input.treecheckbox:checked');
            //    if (cur.length == 0) {
            //        cursend = $(cursend).parents("ul:first").parents('li:first');
            //        if (cursend.length == 0)
            //            conti = false;
            //        $(cursend).children('a').find('input.treecheckbox').prop("checked", false);
            //    }
            //    else
            //        conti = false;
