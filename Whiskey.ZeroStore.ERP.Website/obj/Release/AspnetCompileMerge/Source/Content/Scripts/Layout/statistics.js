...
(function ($) {
    ...
    function message() {
        ...
        me.send = function (content, adminIds, callback) {
            if ($.isFunction(callback)) {
                me.callback = callback;
            }
            if (!isNaN(me.adminId) && me.server && adminIds instanceof Array) {
                me.server.sendMessageTo(me.adminId, content, adminIds);
                me.callback(true, '发送成功');
            } else {
                me.callback(false, '参数无效');
            }
        };
        me.get = function (data) { };
        me.getAllAdmin = function (data) { };
        me.me = function (data) { };
        me.login = function (data) { };
        me.exit = function (adminIds) { };
    }

    ; $.fashion.message = new message();

    ; function notification() {
        var me = this;
        me.show = function (title, content) {
            var $noti = $("<div class='noti-modal noti-effect-13 noti-show' />");
            var $content = $("<div class='noti-content'><h3>" + (title || '通知') + "</h3><div>" + content + "<button class='btn-noti-close' onclick='$(this).parents(\".noti-modal.noti-show:first\").remove();$(\".noti-overlay\").remove();'>好的</button></div></div>");
            if ($(".noti-overlay").length == 0) {
                $("<div class='noti-overlay' />").css("visibility", "visible").appendTo("body");
            }
            $noti.append($content).appendTo("body");
        }
    }
    ; $.fashion.notification = new notification();
 ...
 * 1. check-number
 * 2. check-int
 * 3. check-float【默认2位】 / check-float="3" 保留3位
 * 4. min-value 最小值
 * 5. max-value 最大值
 * 6. dis-drop 禁止拖拽
 * 7. dis-paste 禁止粘贴
 */
    ...
    var keycodeArr = [8, 35, 36, 37, 39];
    //regex
    ; function inputkeyup_number(event) {
        $this = $(this);
        var value = $this.val();
        var keycode = event.keyCode;

        if (!value || keycodeArr.indexOf(keycode) > -1) return;

        var reg = /^(-?\d+\.?\d*)|(-)$/

        var isvalue = !reg.test(value) ? $this.clearVal() : true;

        if (isvalue) {

            var minVal = $this.attr("min-value");
            var maxVal = $this.attr("max-value");

            if (value.indexOf("-") == 0 && value.length == 1) {
                if (!minVal) {
                    return;
                } else if (minVal.indexOf("-") >= 0) {
                    return;
                } else {
                    value = minVal;
                }
            }
            if (value.indexOf(".") == value.length - 1 && !$this.hasAttr("check-int")) return;
            ...
            if (minval && value < minval) { value = minval; isCalc = true }

            if (maxval && value > maxval) { value = maxval; isCalc = true }

            if ($this.hasAttr("check-int")) { value = Math.floor(value); isCalc = true; }

            if ($this.hasAttr("check-float")) {
                var floatlength = parseInt($this.attr("check-float") || 2);
                var dian = tmpVal.indexOf(".");
                if (dian != -1 && tmpVal.substring(dian + 1, tmpVal.length).length > floatlength) {
                    value = tmpVal.substring(0, tmpVal.length - 1);
                    isCalc = true;
                }
            }

            if (!isCalc) value = tmpVal.indexOf(".") != tmpVal.lastIndexOf(".") ? tmpVal.substring(0, tmpVal.length - 1) : tmpVal;

            $this.val(value);
        }
    }

    ; $(document).delegate("input[check-number]", "keyup.ckeck_number", inputkeyup_number);
    ; $(document).delegate("input[check-number]", "blur.ckeck_number", function () {
        var value = this.value;
        if (value.indexOf("-") == 0 && value.length == 1) { value = ""; }
        if (value.indexOf(".") == value.length - 1) { value = value.replace(".", ""); }
        this.value = value;
    });
    ; $(document).delegate("input[dis-drop]", "drop", function () { return false; });
    ; $(document).delegate("input[dis-paste]", "paste", function () { return false; });
