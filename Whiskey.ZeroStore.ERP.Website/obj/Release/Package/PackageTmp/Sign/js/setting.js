/**
 * Created by xiuxia on 2016/7/1.
 */
$(function() {
    $("#footerInfo").height($(window).height() - $(".person_infi").height() - $(".moduleIner").outerHeight());
    //修改个人信息
    ...
    /* 修改MAC 地址页面*/
    $(".macAdd,#change_mac").click(function() {
        $(".MAC_info").animate({
            left : '0'
        })
    });
    ...
    //修改密码页面
    $(".padWord,#change_psd").click(function() {
        $(".password_info").animate({
            left : '0'
        });
    });
    ...
    $(".back_tx").click(function() {
        $(".only_info,.MAC_info,.password_info").animate({
            left : '150%'
        })
    });
    ...
    /*电话号码*/
    $(".numPhone").click(function() {
        $(".number_phone").animate({
            left : '0'
        })
    });
    $(".three_bck").click(function() {
        $(".true_name,.number_phone,.email,.data_of_birth").animate({
            left : '150%'
        })
    });

    $(".idCard,.cardId img").click(function() {
        $(".CordId").animate({
            left : '0'
        });
    });
...
$(function() {
    ...
    //修改生日
    $("#birthday,#birthDay").on("change", function() {
    ...
    $.xiaodie.ajax({
        ...
        success : function(data) {
            if (data.ResultType == 3) {
                $(".only_sub").parent().parent().animate({
                    left : '150%'
                });
            } else {
        ...
        $.xiaodie.ajax({
            ...
            success : function(data) {
                if (data.ResultType == 3) {
                    ...
                    var UserPhoto = $.xiaodie.host + adminInfo.UserPhoto;
