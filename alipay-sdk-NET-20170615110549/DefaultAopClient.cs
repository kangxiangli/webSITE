...
namespace Aop.Api
{
    ...
    public class DefaultAopClient : IAopClient
    {
        ...
        //
        public DefaultAopClient(string serverUrl, string appId, string privateKeyPem, string format, string version, string signType, string alipayPulicKey, string charset, bool keyFromFile, string notify_url)
            : this(serverUrl, appId, privateKeyPem, format, version, signType, alipayPulicKey, charset, keyFromFile)
        {
        ...
        private AopDictionary buildRequestParams<T>(IAopRequest<T> request, String accessToken, String appAuthToken) where T : AopResponse
        {
            ...
            result.Add(NOTIFY_URL, this.notify_url);
            result.Add(CHARSET, charset);
            //result.Add(RETURN_URL, request.GetReturnUrl());
            result.Add(RETURN_URL, RETURN_URL);
