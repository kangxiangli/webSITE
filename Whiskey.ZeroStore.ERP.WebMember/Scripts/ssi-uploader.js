...
(function(root, factory) {
	...
	Ssi_upload.prototype.init = function(element) {
		...
		var $chooseBtn = $('' +
			...
			'</span>').append(element);
		var $uploadBtn = $('<button id="ssi-uploadBtn" class="uploadConfrimBtn ssi-button success ssi-hidden" >' +
		...
		//  this.$element.append($('<div class="ssi-buttonWrapper">');
		this.$element.append($chooseBtn, $abortBtn, $uploadBtn, $clearBtn);
		var $uploadBox;
		...
		$uploadBox.on('mouseenter', '.ssi-statusLabel', function(e) { //the tooltip
			...
			$tooltip = $('<div class="ssi-infoTooltip">' +
				...
				.css({
					top: $eventTarget.position().top - tooltipHeight,
					left: $eventTarget.position().left - 5
				})
				.fadeIn('slow');
			...
			if(thisS.options.preview) {
				var getTemplate = function(content) {
					return '<table class="ssi-imgToUploadTable ssi-pending">' +
						...
						'<tr style="text-align: center;position: ABSOLUTE;bottom: -30px;right: 0;" ><td><button data-delete="' + index + '" class=" ssi-button error ssi-removeBtn"><span class="trash10 trash"></span></button></td></tr>' +
						'</table>'
				};
	...
	Ssi_upload.prototype.uploadFiles = function() { // upload the pending files
		...
		function ajaxLoopRequest(formData, ii) {
			...
			var onProgressHandler = function(event) { // add event listener to progress
				debugger
				if(event.lengthComputable) {
					var percentComplete = (event.loaded / event.total) * 100; // calculate the progress
					if(uploadBar) {
						uploadBar.css({
			...
			$.ajax(ajaxOptions).done(function(responseData, textStatus, jqXHR) {
				...
				if(typeof thisS.options.UploadCallback =="function"){
					thisS.options.UploadCallback(responseData, textStatus, jqXHR);
				}
	...
	var locale = {
		en: {
			...
			clear: '',
			drag: '点击上传或者拖拽图片',
			sizeError: '$1 大小超过限制 $2', // $1=file name ,$2=max ie( example.jpg has has exceed the size limit of 2mb)
			extError: '$1 不支持的文件格式', //$1=file extension ie(exe files are not supported)
