...
function numberValid(contr) {
    ...
    if (!reg.test(conval)) {
        if (conval.length != 0) {
            ...
            $.fashion.web.alert({ type: "warning", content: "输入的内容只能为数字", callback: function () { } })
        }
        else
...
function verifyDat() {
    return "<button display='inline'  style='color:green; margin-right:3px' title='确定入库' type='button' class='btn  glyphicon glyphicon-ok'></button>";
}
function deleDat() {
   
    return "<button display='inline' type='button' style='color: #e5603b'  margin-right:3px' title='删除' class='btn btn_remove glyphicon glyphicon-remove'></button>";
...
function showTooTip(bo,da) {
    if (bo == true || bo == "true") {
        return "<label style='float:left' class='px-single'><button display='inline' attr='"+da+"'  style='color:green; margin-right:3px' type='button' class='btn che_1  glyphicon glyphicon-ok'></button></label>";

        
    }
    else {

        return "<label style='float:left' class='px-single'><button display='inline' attr='" + da + "'  style='color: #e5603b; margin-right:3px'  type='button' class='btn che_1 glyphicon glyphicon-remove'></button></label>";
    }
}
...
function addRow(da) {
    ...
    if (jsonda != undefined && jsonda != null) {
        var res = "";
        res += "<tr class='dt_row'><td class='dt_td dt_ch'>" + $.fashion.datatable.tplListCheckbox(jsonda.Id) + "</td><td class='dt_td dt_num'>" + jsonda.ProductNumber + "</td><td class='dt_td'>" + jsonda.ProductName + "</td>";
        ...
        $($.fashion.datatable.instance[0]).children("tbody").append(res);
    }
