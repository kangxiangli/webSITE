...
namespace Whiskey.Utility.Extensions
{
    ...
    public static class CollectionExtensions
    {
        ...
        /// <typeparam name="T"> 动态类型 </typeparam>
        /// <returns> 为空返回True，不为空返回False </returns>
        public static bool IsNullOrEmptyThis<T>(this IEnumerable<T> collection)
        {
            if (collection.IsNullThis())
                return true;
        ...
        public static bool IsNotNullOrEmptyThis<T>(this IEnumerable<T> collection)
        {
            return !collection.IsNullOrEmptyThis();
            //return collection == null ? false : collection.Any();
        }

        public static bool IsNullThis<T>(this IEnumerable<T> collection)
        {
            return (collection == null || ((collection as IList<T>) == null && collection.ToList() == null));
        }
        public static bool IsNotNullThis<T>(this IEnumerable<T> collection)
        {
            return !collection.IsNullThis();
        }
        ...
        public static void RemoveIf<T>(this IList<T> source, Predicate<T> predicate)
        {
            if (source.IsNullOrEmptyThis() || predicate == null)
                return;
            (source as List<T>).RemoveAll(predicate);
        }
        /// <summary>
        /// 向集合项中添加项
        /// </summary>
        /// <typeparam name="T">动态类型</typeparam>
        /// <param name="source">要添加的源，为空时会自动创建</param>
        /// <param name="addlist">要新添加的项</param>
        public static void AddRanges<T>(this IList<T> source, IEnumerable<T> addlist)
        {
            if (source.IsNullThis() || addlist.IsNullOrEmptyThis())
                return;
            (source as List<T>).AddRange(addlist);
        }
        ...
        public static void ListRandom(this List<int> list, int minValue = 1)
        {
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                if (item > minValue)
                {
                    ...
                    var gval = rd.Next(0, item - minValue + 1);//将被增加的值,+1是因为取不到最大值
                    list[i] -= gval;
                    list[ind] += gval;
                }
            }
        ...
        /// <param name="keySelector">重复数据筛选条件</param>
        /// <returns>不重复元素的集合</returns>
        public static IQueryable<TSource> DistinctQueryBy<TSource, TKey>(this IQueryable<TSource> source, Expression<Func<TSource, TKey>> keySelector)
        {
